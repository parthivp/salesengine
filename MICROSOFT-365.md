...
| `fetch failed` / `ENETUNREACH` | The container cannot reach Microsoft — not a credentials problem. On Alpine this was unfixable: musl resolved `login.microsoftonline.com` to IPv6 addresses only, and Docker Desktop gives containers no IPv6 route. The image is Debian-based for this reason. If you see it again, check `docker compose exec app node -e "require('dns').lookup('login.microsoftonline.com',{all:true},(e,a)=>console.log(a))"` — any `family:4` entry means the resolver is fine and something else is blocking egress (VPN, proxy, firewall). |
...
| "Authenticated, but denied access to that mailbox" | Admin consent was not granted, **or** an Application Access Policy excludes it. The app now tells you which, by reading the roles in its own token. Run `Test-ApplicationAccessPolicy` to confirm the policy half. |
| Denied even though the token shows `Mail.Read` | Fixed — verification used to call a directory endpoint needing `User.Read.All`. If you are on an older build, update. Do **not** add `User.Read.All`: this app has no business reading your directory. |
