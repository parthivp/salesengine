...
| `fetch failed` / `ENETUNREACH` | The container cannot reach Microsoft — not a credentials problem. Usually IPv6: Microsoft publishes AAAA records and Docker Desktop gives containers no IPv6 route. `docker-compose.yml` sets `NODE_OPTIONS=--dns-result-order=ipv4first` on the app and worker to force IPv4; if you overrode it, put it back. |
